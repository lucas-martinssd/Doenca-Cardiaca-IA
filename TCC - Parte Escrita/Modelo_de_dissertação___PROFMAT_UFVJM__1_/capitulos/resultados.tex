...
A Figura~\ref{fig:convergencia_banco2} reforça o diagnóstico de \textit{overfitting} nos modelos MLP, evidenciando ao mesmo tempo um comportamento distinto entre as duas implementações do ADALINE. O ADALINE NumPy apresenta convergência suave e monotônica nas três configurações, coerente com os resultados de acurácia estáveis observados na Seção~\ref{sec:banco2_acuracia}. O ADALINE Sklearn, por sua vez, exibe oscilações visíveis no erro de treinamento, especialmente nas configurações C2 e C3: esse comportamento decorre do otimizador SGD com momento ($\beta = 0{,}9$) operando sobre mini-lotes, em contraste com o gradiente descendente em lote completo e sem momento do ADALINE NumPy (Seção~\ref{sec:discussao_adam}). Atualizações realizadas sobre subconjuntos parciais dos dados, combinadas com o momento acumulado, introduzem variância nas estimativas de gradiente a cada época, efeito que tende a se acentuar em conjuntos de dados pequenos como o Banco 2.
