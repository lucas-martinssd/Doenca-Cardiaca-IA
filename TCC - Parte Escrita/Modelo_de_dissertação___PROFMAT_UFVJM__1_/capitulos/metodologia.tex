...
\begin{itemize}
    \item \textbf{Conjunto de treino/validação}: 70\% dos registros, com a mesma proporção de classes do conjunto original;
    \item \textbf{Conjunto de teste}: 30\% dos registros, mantida a estratificação de classes.
\end{itemize}
...
Ao minimizar essa função via SGD com saída sigmoide, o \texttt{SGDClassifier} com \texttt{loss='log\_loss'} é matematicamente equivalente à \textbf{regressão logística} treinada por gradiente estocástico, e, portanto, arquiteturalmente análogo ao ADALINE Logístico implementado em NumPy \cite{braga2007}. A diferença principal, que impede a equivalência funcional estrita, reside na função de custo: a implementação NumPy minimiza o MSE, enquanto o \texttt{SGDClassifier} com \texttt{log\_loss} minimiza a entropia cruzada, que do ponto de vista da máxima verossimilhança é formalmente mais adequada para classificação binária \cite{silva2016}. A instanciação e o treinamento do modelo seguiram o padrão:
...
Todos os modelos foram implementados na linguagem \textbf{Python 3}, escolhida por seu extenso ecossistema de bibliotecas científicas e por ser a linguagem predominante em pesquisa de aprendizado de máquina. As principais bibliotecas utilizadas foram:

\begin{itemize}
    \item \textbf{NumPy}: computação numérica e álgebra linear, utilizada na implementação manual dos modelos;
    \item \textbf{Pandas}: manipulação e pré-processamento dos conjuntos de dados em formato tabular;
    \item \textbf{Scikit-learn}: modelos com biblioteca (\texttt{SGDClassifier} e \texttt{MLPClassifier}), divisão (\texttt{train\_test\_split}) e métricas (\texttt{accuracy\_score});
    \item \textbf{Matplotlib} e \textbf{Seaborn}: geração de gráficos de curvas de aprendizado e matrizes de confusão.
\end{itemize}
